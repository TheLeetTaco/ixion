...
Run all four locators in a single message with parallel Task calls. Wait for all to complete. Locators have no Read tool — pass search context inline. Return paths/references only — no file contents.
...
Run all four analyzers in parallel where inputs are independent. Each analyzer receives its top-N inputs from Phase 2. All analyzers operate in documentarian mode — flags below are observations (not recommendations), so they remain documentarian-compliant.
