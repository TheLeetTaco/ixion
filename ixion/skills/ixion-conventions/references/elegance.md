...
- **Elegance Dispatch Bar** — the contract every work subagent and every reviewer is held to. Dispatches name this section by path; the agent reads it here.
...
| Anti-Pattern | Signal | Elegant alternative |
|---|---|---|
| **Clone to Satisfy Borrowck** | `.clone()` to silence the borrow checker, not because two owners are needed | Restructure ownership — pass `&T`, split the borrow, or move the value |
| **Stringly-Typed Errors** | `Result<T, String>` or `Box<dyn Error>` in a library API | `thiserror` enum callers can match on |
| **Rc\<RefCell\<T\>\> Masking a Design Cycle** | Shared mutability papering over bidirectional ownership | One owner + borrows, or ids/indices (see Bidirectional Coupling) |
| **MutexGuard Across .await** | Guard held over an await point | Drop the guard before awaiting, or use an async-aware lock |
...
Work dispatches and reviewer dispatches both send agents here by path. Treat the section below as the binding contract for the chunk or diff you were given.
