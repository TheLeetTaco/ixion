...
**When invoked from `ship` Phase 4**, a second source arrives alongside the conversation: the session harvest (what plan review reshaped, which principle names recurred in `review.findings.json`, what the fix pass undid). Treat it the same as conversation context — it fills the same slots:
...
Session-sourced entries are usually `pattern`, `mistake`, `best_practice`, or `workflow_issue` rather than the error-shaped types. The "verified solution" precondition still holds: every chunk passed its own verification and the session's success criteria before `ship` reached this phase.
