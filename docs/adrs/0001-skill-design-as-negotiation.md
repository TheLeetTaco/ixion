...
**Phase 4 grew 3,986 bytes against a 4,000-byte budget, and the budget is why this amendment is long.** Rationale Discipline (`ixion-conventions/SKILL.md`) says justification that does not change what the agent does at runtime belongs here, and `ship/SKILL.md` is Read in full on every invocation. So the skill names five issues by number and spends one clause each on what they mean for the line beside them; the narrative of what those issues are, when they were checked, and which alternatives they killed lives here alone. A reader who wants to know whether #8792 was fixed and the probe can be deleted has one place to look.

`tests/branch-resolution-harness.sh` gains offline coverage of all four `automerge=` values and a case asserting that a declined or failed arming still leaves `status: completed` — the terminal-write invariant, which is what stops the new step from making the session resumable again. **What stays unexercised is the same seam the amendment above named**: `gh pr create` was never faked, and `gh api` and `gh pr merge --auto` are not faked either, because each needs a GitHub remote and a credential and a stub would test the stub. The ladder that computes `automerge=` is covered; the two probes that feed it and the one command that spends it are not. One residual is worth stating because it is easy to mistake for new: that harness is not in `validate.yml`'s shellcheck list and never has been — the gap predates this change and is not created by it.
