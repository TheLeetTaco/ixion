...
Not built, on the evidence available. Two live runs of `07` produced imperfect chains in *different* ways — one dropped `--locked`, the other dropped `--all-targets` and clippy entirely — and the flag that was missing from both, `--all-features`, turned out to be the one the planner was right to omit on a crate with no features. Two samples, one of which the planner got right, is not the "coaxing has demonstrably failed across multiple runs" bar Principle 1 sets, and the change is not small: it makes `work` language-aware in its orchestration, adds `language-standards` to what `work` loads on every invocation against a session that just spent effort reducing exactly that, and changes what `verification` means to `plan-creation`, to the schema's description of the field, and to `07`.

Reopen when a run shows a *fixture-relevant* gate missing again — `--locked` or `--all-targets` on a crate that has a lockfile and tests. `07` asserts both as hard failures and reports `--all-features` as INFO, so the next occurrence is already instrumented, and a third data point either way settles it.
