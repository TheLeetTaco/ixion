...
| `work-review` | `review.findings.json` | Findings reflect what's actually in the diff (read the diff and corroborate). Failure paragraphs lead with principle names. `[Contradicts user]` tags on any finding that pushes back on an explicit user choice. Surviving P1 runtime claims carry the command that reproduced them appended to `failure`; the summary reports the checked/reproduced/refuted/unproven counts. A round where every P1 runtime claim reproduced is suspicious — the gate is meant to kill some. |
